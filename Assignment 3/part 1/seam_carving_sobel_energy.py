...
def compute_energy_map(bgr_image):
    gray_image = cv2.cvtColor(np.uint8(bgr_image), cv2.COLOR_BGR2GRAY)
    ...
    gradient_mag_normalized = cv2.normalize(gradient_mag, None, 0, 255, cv2.NORM_MINMAX)

    gradient_map = np.float32(gradient_mag_normalized)
